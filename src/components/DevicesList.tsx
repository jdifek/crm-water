...
const DevicesList = () => {
	...
	const totalTankVolume = filteredDevices.reduce(
		(sum, device) => sum + device.tankVolume,
		0
	)
	const totalSoldLiters = filteredDevices.reduce(
		(sum, device) => sum + device.soldLiters,
		0
	)
	const totalFilledLiters = filteredDevices.reduce(
		(sum, device) => sum + device.filledLiters,
		0
	)
	const totalRemainingLiters = filteredDevices.reduce(
