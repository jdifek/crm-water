...
const MaintenanceHistory = () => {
	...
	const calculateDaysLeft = (deadline: string) => {
		if (!deadline) return '-'  // если дедлайн не указан, показываем прочерк

		const today = new Date()
		const deadlineDate = new Date(deadline)
		// Если дедлайн равен сегодня, то показываем прочерк
		if (today.toDateString() === deadlineDate.toDateString()) {
			return '-'
		}
		const diffInMs = deadlineDate.getTime() - today.getTime()
		const daysLeft = Math.ceil(diffInMs / (1000 * 60 * 60 * 24))
		return daysLeft >= 0 ? `${daysLeft} дн.` : `-${Math.abs(daysLeft)} дн.`
	}
